...
func TestGetFlowQuotaDataUsesQuotaDataRoleSpecificDimensions(t *testing.T) {
	...
	rootRows, err := GetFlowQuotaData(900, 2000, nil, nil, 0, common.RoleRootUser)
	...
	adminRows, err := GetFlowQuotaData(900, 2000, []string{"alice"}, nil, 0, common.RoleAdminUser)
	require.NoError(t, err)
	...
	selfRows, err := GetFlowQuotaData(900, 2000, nil, nil, 1, common.RoleCommonUser)
	require.NoError(t, err)
